...
import SearchMap from '../components/organisms/SearchMap';
...
const Search = props => {
  const { nearbyLocations, locationFeatures, currentDestination } = props;
      ...
      <SearchProvider value={{ locationFeatures, nearbyLocations }}>
        <SearchTemplate globalData={globalData} currentDestination={currentDestination}>
          ...
          <SearchMapWrapper className={mapFull && 'full'}>
            <MapWrapper>
              <SearchMap 
                fetchedLocations={nearbyLocations} 
...
Search.getInitialProps = async ctx => {
  const { apiService, asPath } = ctx;
  const locationService = new LocationService(apiService);
  const searchCompleteService = new SearchCompleteService(apiService);
  const query = qs.parse(asPath.replace(/\/.*\//, 'name=').replace('?', '&'));
  const { latitude, longitude, name, address } = query;
  let { radius, features } = query;
  ...
  return { nearbyLocations, locationFeatures, currentDestination };
};
